export class Level2 extends Phaser.Scene {
    ...
    create() {
        ...
        this.Level2Map = this.add.image(420,230,'Level2Map');
        this.Level2Map.setScale(.85);
        this.mapTexture = this.textures.get('Level2Map').getSourceImage();
        let key = 'mapCanvas';
        ...
        let y_fin = 320;
        let y_enemy_offset;
        this.PhishNumbers = [87,88,89];
        this.DDOSNumbers = [42];
        this.RansomwareNumbers = [56];
        this.TrojanNumbers = [31];
        for(let i = 0; i < 89; i++) {
            y_enemy_offset = Math.floor(Math.random() * (y_fin - y_start) + y_start);
            x_offset = Math.floor(Math.random() * (x_start - x_fin) + x_fin);
            if (!this.PhishNumbers.includes(i) && !this.DDOSNumbers.includes(i) && !this.RansomwareNumbers.includes(i) &&  !this.TrojanNumbers.includes(i)) {
                ...
                this.Malware.y_enemy_offset = y_enemy_offset;
                ...
                this.Phish.y_enemy_offset = y_enemy_offset;
                ...
                this.DDOS.y_enemy_offset = y_enemy_offset;
                ...
                this.Ransomware.y_enemy_offset = y_enemy_offset;
                ...
                this.Trojan.y_enemy_offset = y_enemy_offset;
    ...
    isGreenPixel(worldX, worldY) {
        const localX = Math.floor((worldX - this.Level2Map.x + (this.Level2Map.displayWidth / 2)) / this.Level2Map.scaleX);
        const localY = Math.floor((worldY - this.Level2Map.y + (this.Level2Map.displayHeight / 2)) / this.Level2Map.scaleY);
    ...
    update(time,delta) {
        ...
        if(this.Enemies.length - (3 - this.lives) == 0) {
            this.lives = 0;
            this.transferString = "Level";
            this.registry.set('Level2Complete', true);
        }
