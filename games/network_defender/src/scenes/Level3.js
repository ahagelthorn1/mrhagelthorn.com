export class Level3 extends Phaser.Scene {
    ...
    create() {
        ...
        this.Level3Map = this.add.image(420,230,'Level3Map');
        this.Level3Map.setScale(.85);
        this.mapTexture = this.textures.get('Level3Map').getSourceImage();
        let key = 'mapCanvas';
        ...
        this.Enemies = [];
        this.PhishNumbers = [36,55,80];
        this.DDOSNumbers = [100,101,102];
        this.RansomwareNumbers = [];
        this.TrojanNumbers = [117,118,119,120];
        let y_offset = -15;
        let x_offset;
        let path = 0
        let pathX = [412,612,818];
        for(let i = 0; i < 120; i++) {
            if (path == 0) {
                path = 1;
            } else if (path == 1) {
    ...
    isGreenPixel(worldX, worldY) {
        const localX = Math.floor((worldX - this.Level3Map.x + (this.Level3Map.displayWidth / 2)) / this.Level3Map.scaleX);
        const localY = Math.floor((worldY - this.Level3Map.y + (this.Level3Map.displayHeight / 2)) / this.Level3Map.scaleY);
    ...
    update(time,delta) {
        ...
        if(this.Enemies.length - (3 - this.lives) == 0) {
            ...
            this.registry.set('Level3Complete', true);
        }
